...
# Приветствие уже отправлено — просим GPT сразу перейти к делу.
_NO_GREETING = (
    "ВАЖНО: приветствие клиенту уже отправлено. Не здоровайся, не представляйся "
    "и не повторяй название компании. Ответь коротко и сразу по существу (2-3 предложения)."
)

_BUTTON_PROMPTS: dict[str, str] = {
    "kp_electro": (
        "Клиент выбрал «КП по электрике». Попроси прислать перечень товаров "
        "или спецификацию для расчёта.\n" + _NO_GREETING
    ),
    "kp_slabot": (
        "Клиент выбрал «КП по слаботочным системам» (СКС, СВН, СКУД). Попроси прислать "
        "перечень оборудования или ТЗ для расчёта.\n" + _NO_GREETING
    ),
}
...
async def _process_message(
    ...
    try:
        ...
        if not button_id and phone not in _greeted and msg_type == "text":
            ...
            # Кладём приветствие в историю, чтобы GPT не здоровался повторно
            _add_to_history(phone, text_body or "Здравствуйте", WELCOME_TEXT)
            return
